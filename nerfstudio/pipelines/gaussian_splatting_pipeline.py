...
@dataclass
class GaussianSplattingPipelineConfigLayered(VanillaPipelineConfig):
    """Dynamic Batch Pipeline Config"""

    _target: Type = field(default_factory=lambda: GaussianSplattingPipelineLayered)
...
class GaussianSplattingPipelineLayered(Pipeline):
    ...
    config: GaussianSplattingPipelineConfigLayered
    datamanager: GaussianSplattingDatamanagerLayered
    dynamic_num_rays_per_batch: int
    ...
    def __init__(
        ...
        super().__init__()
        self.model_path = kwargs["model_path"]
        self.num_layers = sum([1 if os.path.isdir(p) else 0 for p in os.listdir(self.model_path)])
        self.pipelines = []
        ...
        kwargs["orientation_transform"] = orientation_transform
        self.datamanager = GaussianSplattingDatamanagerLayered(kwargs["model_path"], camera_pose_transform)
        self._model = config.model.setup(
            scene_box=self.datamanager.train_dataset.scene_box,
            num_train_data=len(self.datamanager.train_dataset),
            **kwargs,
    ...
    def get_orientation_transform_from_image(self, ref_orientation: str):
        ...
        for subpath in os.listdir(self.model_path):
            if not os.path.isdir(os.path.join(self.model_path, subpath)):
                continue
            cameras_json_path = os.path.join(self.model_path, subpath, "cameras.json")
    ...
    def get_orientation_transform_by_up(self):
        ...
        for subpath in os.listdir(self.model_path):
            ...
            cameras_json_path = os.path.join(self.model_path, subpath, "cameras.json")
            if os.path.exists(cameras_json_path):
                print("cameras_json_path", cameras_json_path, "does not exist")
                break
        if os.path.exists(cameras_json_path) is False:
            raise RuntimeError("{} not exists".format(cameras_json_path))
        with open(cameras_json_path, "r") as f:
        ...
        up = up / np.linalg.norm(up)
        R = GaussianSplattingPipelineLayered.rotmat(up, [0, 0, 1])  # rotate up vector to [0,0,1]
        R = np.pad(R, [0, 1])
        R[-1, -1] = 1
    ...
    def rotmat(a, b):
        ...
        # handle exception for the opposite direction input
        if c < -1 + 1e-10:
            return GaussianSplattingPipelineLayered.rotmat(a + np.random.uniform(-1e-2, 1e-2, 3), b)
        s = np.linalg.norm(v)
        kmat = np.array([[0, -v[2], v[1]], [v[2], 0, -v[0]], [-v[1], v[0], 0]])
        return np.eye(3) + kmat + kmat.dot(kmat) * ((1 - c) / (s ** 2 + 1e-10))
