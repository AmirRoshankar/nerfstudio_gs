...
class GaussianSplattingConfigLayered:
    config: TrainerConfig = TrainerConfig(
        method_name="gaussian_splatting_layered",
        steps_per_eval_batch=999999999,
        steps_per_save=999999999,
        max_num_iterations=999999999,
        mixed_precision=True,
        pipeline=GaussianSplattingPipelineConfigLayered(
            datamanager=GaussianSplattingDatamanagerConfigLayered(),
            model=GaussianSplattingModelConfigLayered(
                eval_num_rays_per_chunk=1,
            ),
        ),
        optimizers={},
